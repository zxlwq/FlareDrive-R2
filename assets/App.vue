<template>
  <div class="main" @dragenter.prevent @dragover.prevent @drop.prevent="onDrop">
      ...
      <ul class="file-list">
        <li v-if="cwd !== ''">
          <div tabindex="0" class="file-item" @click="cwd = cwd.replace(/[^\/]+\/$/, '')" @contextmenu.prevent>
            <div class="file-icon">
              <svg t="1741761327819" class="icon" viewBox="0 0 1024 1024" version="1.1"
                xmlns="http://www.w3.org/2000/svg" p-id="10496" width="36" height="36">
                <path
    ...
    <Dialog v-model="showContextMenu">
      <div
        style="display: flex; justify-content: center; align-items: center; padding: 10px; background: #ddd; margin: 10px; border-radius: 8px;">
        <p style="margin: 0;">信息：</p>
        <div v-text="focusedItem.key || focusedItem" class="contextmenu-filename" @click.stop.prevent
          style="margin-left: 10px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"></div>
      </div>
